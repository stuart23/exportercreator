...
func (ec *exporterCreator) Start(ctx context.Context, h component.Host) error {
	...
	// reports can be attributed to it; they all delegate to the one shared handler.
	for observerID, observable := range observers {
		notify := newObserverNotify(ec.params.ID, observerID, ec.observerHandler, ec.telemetry)
